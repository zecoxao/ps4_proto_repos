...
static void cmd_PASV_func(ftps4_client_info_t *client)
{
	...
	ret = sceNetBind(client->data_sockfd,
		(const struct sceNetSockaddr *)&client->data_sockaddr,
	...
	sceNetGetsockname(client->data_sockfd, (const struct sceNetSockaddr *)&picked,
		&namelen);
...
static void client_open_data_connection(ftps4_client_info_t *client)
{
	...
	if (client->data_con_type == FTP_DATA_CONNECTION_ACTIVE) {
		...
		client->pasv_sockfd = sceNetAccept(client->data_sockfd,
			(const struct sceNetSockaddr *)&client->pasv_sockaddr,
			&addrlen);
			...
			if (dent->d_name[0] != '\0') {
				...
				if (err == 0) {
					...
					if (S_ISLNK(st.st_mode)) {
						if ((readlinkerr = syscall(58,full_path, link_path, sizeof(link_path))) > 0) {
...
static void *client_thread(void *arg)
{
	...
	while (1) {
		...
		if (client->n_recv > 0) {
			...
			printf("\t%i> %s", client->num, client->recv_buffer);
...
static void *server_thread(void *arg)
{
	...
	while (1) {
		...
		if (client_sockfd >= 0) {
			...
			printf("Client %i connected, IP: %s port: %i\n",
